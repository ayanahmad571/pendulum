...
foreach($_POST as $key=>$v){
	...
	if(!is_array($_POST[$key])){
		if($key == 'send_contact_email' or $key == 'disapprove_story_reason'){
			$_POST[$key] =str_replace('\\r','',str_replace('\\n','',str_replace('script>','', trim((($conn->escape_string($v)))))));
		}else{
...
if(strtolower(trim($row['mo_href'])) == 'admin_stories.php'){
		$trem = getdatafromsql($conn,"select count(sto_id) as changes from `tpt_stories` where 
		sto_approved = 0 and sto_valid =1 ");
		if(is_array($trem)){
...
if(strtolower(trim($row['mo_href'])) == 'admin_contact.php'){
	$trrem = getdatafromsql($conn,"select count(ct_id) as dddda from `tpt_contact` where `ct_replied` = 0 and `ct_valid` =1 ");
		if(is_array($trrem)){
...
if ($act_res->num_rows == 1) {
	...
	##if strtolower(mo_href)== reports.php
		...
		';
				}else if(strtolower(trim($row['mo_href'])) == 'admin_contact.php'){
					echo '
